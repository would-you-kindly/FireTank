...
namespace FireSafety
{
    public class WaterTrace : Drawable
    {
        ...
        public WaterTrace(bool up, Vector2f position, int pressure, float degrees)
        {
            ...
            {
                trace.Size = new Vector2f(pressure * (float)Math.Sqrt(Math.Pow(Utilities.GetInstance().TILE_SIZE, 2.0) + Math.Pow(Utilities.GetInstance().TILE_SIZE, 2.0)), 1);
            }
            else
            {
                trace.Size = new Vector2f(pressure * Utilities.GetInstance().TILE_SIZE, 1);
            }
